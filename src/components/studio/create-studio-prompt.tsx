...
interface CreateStudioPromptProps {
  isFirstStudio?: boolean;
}

export function CreateStudioPrompt({ isFirstStudio = false }: CreateStudioPromptProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/studio', {
        ...
        body: JSON.stringify({ name: isFirstStudio ? 'Down Home' : name.trim() }),
      });
        ...
        <div className="w-full max-w-md space-y-6 rounded-xl border border-border/50 bg-surface p-8">
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Your studio is ready. Let&apos;s get started.
            </p>
          </div>
