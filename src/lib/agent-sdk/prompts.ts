...
3. **Ingest (Conditional)**: If the script was pre-ingested via file upload (indicated by "[UPLOADED SCRIPT FILE: ...]" in the message), skip this step — do NOT call ingest_script. Only call ingest_script when script text is pasted directly without the upload marker. When calling it, include the script text, metadata, AND projectId/draftId for memory tracking.
